...
/**
 * Aggregate dashboard card across all debt sources (SPayLater, GoPay
 * Pinjam, ...). Per-debt breakdown lives on the dedicated /debt screen.
 */
      ...
      <Link href="/debt">
        <Card variant="glow" className="cursor-pointer">
          <div className="flex items-center gap-2 text-emerald">
            <PartyPopper size={18} />
            <span className="font-medium">All debts paid off!</span>
          </div>
        </Card>
      </Link>
    );
      ...
      <Card className="cursor-pointer transition hover:border-gold/30">
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-ink">Debt-Free Progress</span>
          </div>
          <span className="font-mono text-sm text-ink-muted">{summary.progressPct}% paid</span>
        </div>
        <ProgressBar progress={summary.progressPct} color="#34D399" />
        <div className="mt-3 flex items-center justify-between text-sm">
          <span className="text-ink-muted">{formatRupiahCompact(summary.remainingAmount)} left</span>
          <span className="text-ink-faint">Target {countdown.targetLabel}</span>
        </div>
        {summary.nextDueDate && (
          <p className="mt-2 text-xs text-ink-faint">
            Next due {format(parseISO(summary.nextDueDate), "d MMM")} ·{" "}
